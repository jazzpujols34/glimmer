...
async def process_export(request: ExportRequest, work_dir: Path) -> tuple[bool, str, Optional[Path]]:
    ...
    if request.titleCard:
        ...
        drawtext = f"drawtext=text='{escape_ffmpeg_text(tc.text)}':fontsize=48:fontcolor={text_color}:x=(w-text_w)/2:y=(h-text_h)/2"
        if tc.subtitle:
            drawtext += f",drawtext=text='{escape_ffmpeg_text(tc.subtitle)}':fontsize=28:fontcolor={text_color}:x=(w-text_w)/2:y=(h+text_h)/2+20"
    ...
    if request.outroCard:
        ...
        drawtext = f"drawtext=text='{escape_ffmpeg_text(oc.text)}':fontsize=48:fontcolor={text_color}:x=(w-text_w)/2:y=(h-text_h)/2"
        if oc.subtitle:
            drawtext += f",drawtext=text='{escape_ffmpeg_text(oc.subtitle)}':fontsize=28:fontcolor={text_color}:x=(w-text_w)/2:y=(h+text_h)/2+20"

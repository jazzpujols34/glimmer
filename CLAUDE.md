...
- **[2026-08-02] Progressive Google identity Phase 2b enforcement shipped DORMANT** (branch `feature/oauth-2b-enforcement`, not yet merged/deployed) — `enforceIdentity()` (`src/lib/identity.ts`) is wired into the spend/destructive routes (`generate`, `generate-batch`, `quick-generate`, `transcribe`, and the DELETE/batch-delete handlers on `gallery`/`projects`/`storyboards`) behind `REQUIRE_SESSION_FOR_PAID`, default OFF — with the flag off every wired route is byte-identical to pre-Phase-2b (all 366 pre-existing tests stayed green with zero changes, flag unset). Auto-migrate model: the OAuth callback now also writes `sessionreq:<email>` at sign-in, so an account only becomes session-required once it has actually signed in — a legacy/never-signed-in email is never locked out by flipping the flag on later. GET/list reads deliberately left unwired (deferred, lower lockout risk). Bundle: 24.899 → 24.933 MiB, ~67 KiB headroom left.

...
- **[2026-03-02] Video Generation Timing**: BytePlus Seedance takes **2-5 minutes** per video after submission. Videos won't appear in the gallery immediately — tell the user to wait a few minutes and refresh. The batch script submits jobs but doesn't wait for completion. Use `node scripts/batch-status.mjs --base-url https://glimmer.video` or `curl https://glimmer.video/api/status/<jobId>` to check progress.

- **[2026-03-03] Gallery vs Job Page Polling**: The `/generate/[jobId]` page shows videos fast because it polls `/api/status/[id]`, which **actively checks the external provider** and updates KV. The `/gallery` page only calls `getCompletedJobs()` which **reads KV without checking providers** — so jobs stay `processing` until someone polls the status endpoint. This is the client-driven polling architecture (Edge Runtime can't have background workers). **Solution:** Add a "Refresh" button to gallery that bulk-polls `/api/status` for all processing jobs, triggering external checks and KV updates.

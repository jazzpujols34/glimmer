...
| `REQUIRE_SESSION_FOR_PAID` | optional, **default off** | Phase 2b server-side enforcement (`src/lib/identity.ts` `enforceIdentity()`, docs/oauth-identity-design.html §5-6). Unset/not `"true"` = pure no-op, every route behaves exactly as pre-Phase-2b. Set to `true` to require a valid session for spend/destructive actions (`generate`, `generate-batch`, `quick-generate`, `transcribe`, and the DELETE/batch-delete handlers on `gallery`, `projects`, `storyboards`) — but ONLY for accounts that have already signed in at least once (`sessionreq:<email>`, written at Google OAuth callback). This is the auto-migrate anti-spoof lock: an account that has never established a session is never affected by flipping this on, so it can't lock out legacy/anonymous users. Flipping it on is a deliberate rollout step, not part of any code deploy — do it consciously after Google sign-in has been live long enough that active paid accounts have had a chance to sign in once. |
